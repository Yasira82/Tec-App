...
function StatCard({ icon, label, value, color = '#d4af37' }: {
  icon: string; label: string; value: string | number; color?: string;
}) {
  return (
    <div style={s({ background: '#0d0d14', border: `1px solid ${color}20`, borderRadius: 18, padding: '16px 20px' })}>
...
export default function AnalyticsPage() {
  ...
  return (
    <div style={s({ padding: '24px 16px', maxWidth: 600, margin: '0 auto' })}>
      ...
      {/* 7-Day Metrics */}
      {metrics.length > 0 && (
        <div style={s({ background: '#0d0d14', border: '1px solid #ffffff08', borderRadius: 18, padding: '20px', marginBottom: 16 })}>
          ...
          {metrics.map((m, i) => (
            <div key={i} style={s({ marginBottom: 20, paddingBottom: 20, borderBottom: i < metrics.length - 1 ? '1px solid #ffffff06' : 'none' })}>
              ...
              <MetricBar label="Volume π"  value={m.total_volume}   max={maxVol}   color="#d4af37" />
      ...
      {metrics.length === 0 && !error && (
        <div style={s({ textAlign: 'center', padding: '48px 20px', background: '#0d0d14', border: '1px solid #ffffff08', borderRadius: 18 })}>
