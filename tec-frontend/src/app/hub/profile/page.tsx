    ...
    <HubSubShell title={t.hub.profile.title} subtitle={t.hub.profile.subtitle}>
      ...
      {user?.role === 'admin' && (
        <DashboardCard title={t.hub.profile.admin}>
          ...
          <button onClick={() => router.push('/hub/admin/campaign')}
            ...
            <span style={{ fontSize: 22 }}>π</span>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 'var(--text-sm)', fontWeight: 700, color: 'var(--tec-text-1)' }}>Campaign payouts</div>
              <div style={{ fontSize: 'var(--text-xs)', color: 'var(--tec-text-3)' }}>Who is waiting for Pi</div>
            </div>
            <span style={{ fontSize: 'var(--text-sm)', color: 'var(--tec-gold)' }}>→</span>
          </button>
