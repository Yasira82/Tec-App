...
// getUserMedia only works in a SECURE context with the API present. In the Pi
// Browser (a mobile webview) it is usually blocked — so the reliable capture
// path there is the native <input capture="user"> (opens the phone's front
// camera directly). We only OFFER the in-app live camera where it can actually
// run, and never depend on it.
...
// If the live camera doesn't start within this window we stop waiting, so the
// full-screen modal can NEVER hang black on top of the rest of the form.
const CAMERA_START_TIMEOUT_MS = 8000;

// KYC selfie capture.
//
// PRIMARY (works everywhere, incl. Pi Browser): the tile is a full-size
// transparent <input type="file" capture="user"> overlay — a direct tap opens
// the phone's FRONT camera to shoot the face (same robust pattern as the ID
// photo fields). No programmatic click, no webview quirks.
//
// ENHANCEMENT (desktop / browsers that allow getUserMedia): a secondary
// "live camera" button opens an in-app mirrored preview with a capture button.
// It has a watchdog + explicit fallback so a blocked/slow camera degrades to
// the native input instead of trapping the user.
export function LiveSelfieCapture({ label, required, initialValue, onChange }: Props) {
  ...
  // Only decide whether to show the live-camera button AFTER mount — hasLiveCamera()
  // is false during SSR, so gating on it directly would cause a hydration mismatch.
  ...
  // Always release the camera + timer when the modal closes or the component unmounts.
  ...
  const openLiveCamera = useCallback(async () => {
    ...
    setStarting(true);
    setCamOpen(true);
    // Watchdog: if the camera hasn't started in time, surface the fallback
    // instead of leaving a black modal covering the form.
    clearWatchdog();
  ...
  const capture = useCallback(() => {
    ...
    // feels natural; the stored evidence keeps real orientation.
  ...
  return (
    <div style={{ marginBottom: 'var(--sp-5)' }}>
      ...
      {/* PRIMARY: full-size transparent <input capture="user"> overlay — a direct
          tap opens the phone's front camera (reliable in the Pi Browser). */}
      <label
        style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, width: '100%', pointerEvents: 'none' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            {state === 'uploading' ? (
              ...
              <>
                <div style={{ fontSize: 'var(--text-sm)', color: 'var(--tec-text-1)', fontWeight: 600 }}>Take a selfie</div>
      ...
      {/* ENHANCEMENT: in-app live camera — only where getUserMedia can run. */}
      {mounted && hasLiveCamera() && (
        <button
          ...
          <Icon name="camera" size={13} color="var(--tec-gold)" /> Or open the live camera in-app
        </button>
      )}
          ...
          {camErr ? (
            ...
            <>
              ...
              <div style={{ display: 'flex', gap: 'var(--sp-4)', alignItems: 'center' }}>
                ...
                <button type="button" onClick={capture} disabled={starting} aria-label="Capture selfie"
                  style={{
